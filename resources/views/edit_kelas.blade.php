  ...
  <div class="bg-white w-full max-w-2xl p-6 rounded-xl shadow-md">
    ...
    <form action="{{ route('kelas.update', $kelas->kode_kelas ) }}" method="POST" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="kode_kelas" value="{{ $kelas->kode_kelas }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required readonly>
      </div>

      <div>
        <label class="block text-sm font-semibold text-gray-600 mb-1">Nama Kelas</label>
        <input type="text" name="nama_kelas" value="{{ $kelas->nama_kelas }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required>
      </div>
