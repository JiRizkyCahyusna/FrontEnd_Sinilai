  ...
  <div class="bg-white w-full max-w-2xl p-6 rounded-xl shadow-md">
    ...
    <form action="{{ route('mahasiswa.update', $mahasiswa->npm) }}" method="POST" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="npm" value="{{ $mahasiswa->npm }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required readonly>
      </div>

      <div>
        <label class="block text-sm font-semibold text-gray-600 mb-1">Nama</label>
        <input type="text" name="nama_mhs" value="{{ $mahasiswa->nama_mhs }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required>
      </div>

      <div>
        <label class="block text-sm font-semibold text-gray-600 mb-1">Kode Kelas</label>
        <input type="text" name="kode_kelas" value="{{ $mahasiswa->kode_kelas }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required>
      </div>

      <div>
        <label class="block text-sm font-semibold text-gray-600 mb-1">ID prodi</label>
        <input type="text" name="id_prodi" value="{{ $mahasiswa->id_prodi }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-yellow-400" required>
      </div>
